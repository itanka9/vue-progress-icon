  ...
  computed: {
    clipRect() {
      ...
      const hoffset = this.hoffset,
        ...
        vsize =
          dir === "vertical" ? (scaledHeight * progress) / 100 : scaledHeight,
        hsize =
          dir === "horizontal" ? (scaledWidth * progress) / 100 : scaledWidth;

      return `rect(${scaledHeight - vsize + vgap + voffset}px,${
        hsize + hgap + hoffset
      }px,${scaledHeight + voffset + vgap}px,${hoffset + hgap}px)`;
    },
